...
import io.zeebe.test.integration.containers.configurators.BrokerConfiguratorChain;
import io.zeebe.test.integration.containers.configurators.GatewayConfiguratorChain;
...
public final class DockerCluster extends ExternalResource implements Cluster {
  ...
  public DockerCluster(final String version) {
    this(version, new ClusterCfg());
  }

  public DockerCluster(final String version, final ClusterCfg clusterConfig) {
  ...
  public DockerCluster withBrokerConfigurator(final BrokerConfigurator configurator) {
    brokerConfigurators.add(configurator);
    return this;
  }

  public DockerCluster withGatewayConfigurator(final GatewayConfigurator configurator) {
    gatewayConfigurators.add(configurator);
    return this;
  }

  public DockerCluster withNetwork(final Network network) {
  ...
  private ZeebeBrokerContainer createBroker(final int nodeId) {
    final var logger =
        LoggerFactory.getLogger(DockerCluster.class.getName() + ".brokers." + nodeId);
  ...
  private ZeebeStandaloneGatewayContainer createGateway() {
    final var logger = LoggerFactory.getLogger(DockerCluster.class.getName() + ".gateway");
