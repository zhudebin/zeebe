...
public final class FrameUtil {
   ...
   * If the frame should be ignored, the returned Optional is empty. In both cases, the buffer's
   * position remains the same.
   */
  public static Optional<Integer> readVersion(final ByteBuffer buffer, final int offset) {
    final byte val = buffer.get(offset);
    if (val == IGNORE) {
