...
public final class TransformContext {
  ...
  private final Map<DirectBuffer, ExecutableProcess> processs = new HashMap<>();
  ...
  public void addProcess(final ExecutableProcess process) {
    processs.put(process.getId(), process);
  }

  public ExecutableProcess getProcess(final String id) {
    return processs.get(wrapString(id));
  }

  public List<ExecutableProcess> getProcesss() {
    return new ArrayList<>(processs.values());
  }
