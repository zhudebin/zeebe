...
public final class BpmnBufferedMessageStartEventBehavior {
    ...
    findNextMessageToCorrelate(process, correlationKey)
        .ifPresentOrElse(
            messageCorrelation -> {
              ...
              correlateMessage(process, messageCorrelation.elementId, message);
            },
            () -> {
            ...
            message -> {
              ...
              if (message.getDeadline() > ActorClock.currentTimeMillis()
                  && !messageState.existMessageCorrelation(
                      message.getKey(), process.getBpmnProcessId())) {
  ...
  private void correlateMessage(
      final DeployedProcess process, final DirectBuffer elementId, final Message message) {

    final var processInstanceKey =
        eventHandle.triggerStartEvent(
            streamWriter, process.getKey(), elementId, message.getVariables());

    if (processInstanceKey > 0) {
      // mark the message as correlated
      messageState.putMessageCorrelation(message.getKey(), process.getBpmnProcessId());
      messageState.putProcessInstanceCorrelationKey(
          processInstanceKey, message.getCorrelationKey());
    }
  }
