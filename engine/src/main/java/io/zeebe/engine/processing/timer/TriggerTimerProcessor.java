...
public final class TriggerTimerProcessor implements TypedRecordProcessor<TimerRecord> {
  ...
  private void processTimerTrigger(
    ...
    final var processKey = timer.getProcessKey();
    final var catchEvent =
        processState.getFlowElement(
            processKey, timer.getTargetElementIdBuffer(), ExecutableCatchEvent.class);

    final boolean isTriggered =
        triggerEvent(streamWriter, timer, elementInstanceKey, processKey, catchEvent);
    ...
    if (elementInstanceKey > 0) {
      ...
      final var processInstanceKey =
          eventHandle.triggerStartEvent(
              streamWriter, processKey, timer.getTargetElementIdBuffer(), NO_VARIABLES);
