...
public final class CreateProcessInstanceProcessor
  ...
  private boolean setVariablesFromDocument(
    ...
    try {
      variablesState.setVariablesLocalFromDocument(
          processInstanceKey, processKey, record.getVariablesBuffer());
  ...
  private ElementInstance createElementInstance(
    ...
    newProcessInstance.setProcessKey(process.getKey());
  ...
  private DeployedProcess getProcess(
    ...
    } else if (record.getProcessKey() >= 0) {
      process = getProcess(record.getProcessKey(), controller);
    } else {
