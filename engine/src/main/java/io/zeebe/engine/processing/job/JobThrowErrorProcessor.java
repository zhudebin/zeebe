...
public class JobThrowErrorProcessor implements CommandProcessor<JobRecord> {
  ...
  private final CatchEventAnalyzer structureAnalyzer;
  ...
  public JobThrowErrorProcessor(final ZeebeState state) {
    ...
    structureAnalyzer = new CatchEventAnalyzer(state.getWorkflowState(), elementInstanceState);
  }
  ...
  private void acceptCommand(
    ...
    final var foundCatchEvent = structureAnalyzer.findCatchEvent(errorCode, serviceTaskInstance);
    ...
    if (foundCatchEvent == null) {
      ...
      commandControl.reject(
          RejectionType.INVALID_STATE, "Service task is not active " + serviceTaskInstance);
    } else if (!eventScopeInstanceState.isAcceptingEvent(
        foundCatchEvent.getElementInstance().getKey())) {
      commandControl.reject(
          RejectionType.INVALID_STATE,
          "Event scope is not accepting events " + foundCatchEvent.getElementInstance());
    } else {
      commandControl.accept(JobIntent.ERROR_THROWN, job);
    }
  }
