...
public class RaftFailOverTest {
  ...
  public void shouldReplicateEntriesAfterSnapshotOnJoin() throws Exception {
    ...
    for (final String member : memberLogs.keySet()) {
      if (!follower.equals(member)) {
        ...
        assertThat(memberEntries).endsWith(entries.toArray(new RaftLogEntry[0]));
      }
    }
  }
  ...
  private void assertMemberLogs(final Map<String, List<RaftLogEntry>> memberLog) {
    ...
    if (iterator.hasNext()) {
      ...
      while (iterator.hasNext()) {
        ...
        assertThat(firstMemberEntries)
            ...
            .containsExactly(otherEntries.toArray(new RaftLogEntry[0]));
      }
    }
  }
}
