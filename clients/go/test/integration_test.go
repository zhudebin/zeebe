...
func (s *integrationTestSuite) TestCreateInstance() {
	...
	s.EqualValues(process.GetProcessKey(), processInstance.GetProcessKey())
...
func (s *integrationTestSuite) TestActivateJobs() {
	...
	process := deployment.GetProcesss()[0]
	_, err = s.client.NewCreateInstanceCommand().ProcessKey(process.GetProcessKey()).Send(ctx)
	if err != nil {
		s.T().Fatal(err)
	}
	...
	for _, job := range jobs {
		s.EqualValues(process.GetProcessKey(), job.GetProcessKey())
