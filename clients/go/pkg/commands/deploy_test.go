...
func TestDeployCommand_AddResourceFile(t *testing.T) {
	...
	demoName := "../../../java/src/test/resources/processs/demo-process.bpmn"
	demoBytes := readBytes(t, demoName)
	anotherName := "../../../java/src/test/resources/processs/another-demo-process.bpmn"
	anotherBytes := readBytes(t, anotherName)
	yamlName := "../../../java/src/test/resources/processs/simple-process.yaml"
	yamlBytes := readBytes(t, yamlName)

	request := &pb.DeployProcessRequest{
		Processs: []*pb.ProcessRequestObject{
			{
				Name:       demoName,
