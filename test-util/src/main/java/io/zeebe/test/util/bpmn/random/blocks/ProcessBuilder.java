...
public final class ProcessBuilder {
  ...
  private void executionPathForEventSubProcess(
    ...
    if (size < 1) {
      // empty path
      // We will not add here an event sub process, since the likelihood that the event sub process
      // can be triggered in time, before the process is completed, is quite low. This can cause
      // flaky tests.
      return;
    }
